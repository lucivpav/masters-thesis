...
\begin{figure*}[htb!]
	...
	\hline
	\makecell{Query 16 \\ 0.13 m, 1.19$\degree$} &
	...
	\hline
	\makecell{Query 26 \\ 0.48 m, 1.60$\degree$} &
	...
	\hline
	\makecell{Query 31 \\ 0.13 m, 1.17$\degree$} &
	...
	\hline
	\makecell{Query 38 \\ 0.08 m, 1.42$\degree$} &
	...
	\hline
	\makecell{Query 40 \\ 7.84 m, 153.09$\degree$} &
...
\begin{table}[htb!]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		...
		\hline
	\end{tabular}
	\caption{Statistics of the HoloLens1 pose estimation errors. InLocCIIRC got completely lost 29 out of 350 times for all methods. Not included in the mean/median/std errors. Errors are computed by comparing InLocCIIRC pose estimates with reference poses. The errors in [m] units are translation errors and the errors in [$\degree$] units are orientation errors. Lowest errors are highlighted in bold. MCP stands for MultiCameraPose.}
...
\begin{figure}[htb!]
	...
	\caption{Comparison between different methods (and segment lengths) of InLocCIIRC on the HoloLens1 query set. The x-axis describes the maximum allowed translation error. The angular threshold is set to $10\degree$. \todo[this plot is ugly.]}
	\label{fig:HL1-dist-thresh-vs-accuracy}
\end{figure}

\subsection{Best method vs the non-sequential method}
\begin{itemize}
	\item \todo[query pipeline on the best method]
\end{itemize}
...
\chapter{Conclusion}
\todo[I should mention that all the methods are still far away from being better than the original HL poses!]
