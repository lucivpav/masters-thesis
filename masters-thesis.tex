...
\todo[]. Tys honí až nevrlí komise omylem kontor město sbírku a koutě, pán nu lež, slzy, nemají zasvé šťasten. Tetě veselá. Vem lépe ty jí cíp vrhá. Novinám prachy kabát. Býti čaj via pakujte přeli, dyť do chuť kroutí kolínský bába odkrouhnul. Flámech trofej, z co samotou úst líp pud myslel vocaď víc doživotního, andulo a pakáž kadaníkovi. Čímž protiva v žába vězí duní.
...
\todo[Relevant work with regards to HoloLens or indoor localization.]
...
\begin{figure}
	\centering
 	...
 	\caption{Visualization of the HoloLens and Vicon timelines. The synchronization constant must be found. Note that the sampling frequencies are vastly different. However, given a query image from HoloLens taken at some point in time (HoloLens sampling frequency), we find the corresponding reference pose that has the nearest timestamp (after taking the synchronization constant into acount; Vicon sampling frequency).}
...
The algorithm in step \ref{item:topGV-sequential} may be a bit problematic for two reasons. First, some query-cutout pairs may naturally have more inliers (on average) than others. It might be sub-optimal to sum those scores. Instead, e.g. an average or a median should be considered. The second issue is \todo[in evaluation, where I discuss the source of errors, mention that issue where suboptimal query-cutout pairs are chosen in the top \topPE{} combinations having a (small) negative impact on the accuracy]. The problem is that selecting \topPE{} combinations from $\topGV{}^{k}$ is not enough. But increasing the number of chosen top combinations is currently not possible, because pose verification is so slow. Viz OneNote.

\todo[describe other changes from the original demo.]

\chapter{Evaluation}

\todo[This chapter currently only describes the s10e evaluation (non-sequential images taken from the phone).]
...
\begin{figure*}
	...
	\hline
	\makecell{Query 40 \\ 7.81 m, 152.38$\degree$} &
...
\begin{table}
	...
	\begin{tabular}{|c|c|c|c|}
		\hline
		\diagbox{Method}{Threshold} & 0.25m & 0.50m & 1.00m \\
		\hline
		k=1 (non-sequential) & 38.9\% & 56.5\% & 69.9\% \\
		\hline
		k=2, sequentialPV & 60.42\% & 79.14\% & 83.13\% \\
		\hline
		k=3, sequentialPV & 60.74\% & 80.06\% & 83.13\% \\
		\hline
		k=4, sequentialPV & 58.90\% & 79.14\% & 81.90\% \\
		\hline
		k=2, MultiCameraPose & 65.03\% & \bfseries 80.37\% & \bfseries 83.74\% \\
		\hline
		k=3, MultiCameraPose & \bfseries 65.34\% & 78.22\% & 82.82\% \\
		\hline
		k=5, MultiCameraPose & 63.80\% & 77.30\% & 82.20\% \\
		\hline
	\end{tabular}
	\caption{Evaluation of performance of localization methods on HoloLens1 query set. Ran on the InLocCIIRC dataset obviously. Percentage rate of correctly localized queries within given threshold is shown. Angular threshold is equal to $10\degree$ in every row. InMap queries are queries for which we have a similar cutout in the dataset. \todo[evaluate estimated poses by procrustes with poses from HoloLens] (if the queries are from HoloLens). \todo[what would be the <poses from HL wrt Omega converted to be wrt World> InLoc performance? I am sure it would win, lol.]}
	\label{tab:HL1-estimation-performance}
\end{table}

%\subsection{Non-sequential method}
%
%
%\subsection{SequentialPV method}
%\subsubsection{k=2}
%\subsubsection{k=3}
%\subsubsection{k=4}
%
%\subsection{MultiCameraPose method}
%\subsubsection{k=2}
%\subsubsection{k=3}
%\subsubsection{k=5}

\subsection{Best method vs the non-sequential method}
